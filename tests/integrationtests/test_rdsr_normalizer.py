...
from pyskindose.constants import KEY_NORMALIZATION_FILTER_SIZE_ALUMINUM, KEY_NORMALIZATION_FILTER_SIZE_COPPER, \
    KEY_RDSR_FILTER_MATERIAL, KEY_RDSR_FILTER_TYPE, KEY_RDSR_FILTER_MIN, KEY_RDSR_FILTER_MAX
from pyskindose.rdsr_normalizer import rdsr_normalizer


def test_that_multiple_xray_filter_materials_are_extracted_during_normalization_if_present(allura_parsed, axiom_artis_parsed):
    # Arrange
...
def test_xray_filter_mean_thickness_calculated_for_each_filter_material_separately(allura_parsed):
    # Arrange
    ...
    # Act
    data_norm = rdsr_normalizer(data_parsed=allura_parsed)
...
def test_rdsr_normalizer_correctly_handles_events_without_filter(axiom_artis_parsed):
    # Arrange
    expected_aluminium_filter_thicknesses = [0.0] * 21
    expected_copper_filter_thicknesses = [0.0, 0.9, 0.9, 0.9, 0.9, 0.9, 0.6, 0.9, 0.9, 0.9, 0.9, 0.9, 0.6, 0.9, 0.9, 0.3, 0.6, 0.3, 0.9, 0.9, 0.6]
    ...
    # Act
    normalized_data: pd.DataFrame = rdsr_normalizer(data_parsed=parsed_data_with_nofilter_events)
