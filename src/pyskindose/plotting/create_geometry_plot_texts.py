...
def create_geometry_plot_texts(beam: Beam, table: Phantom, pad: Phantom,
                               patient: Optional[Phantom] = None, ) -> Tuple[
        List[str], List[str], List[str], List[str], List[str], Optional[List[str]]]:
    """ Create lists text strings to show when hovering in geometry plots

    Parameters
    ----------
    beam : Beam
        Instance of the Beam class containing info needed for the X-ray source,
        beam and detector texts
    table : Phantom
