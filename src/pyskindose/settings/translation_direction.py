...
class TranslationDirection:
    ...
    def update_translation_direction(self, directions: Dict[str, str]):
        self.x = self._get_direction_as_value(directions["x"]) if self.x is None else self._get_direction_as_value(self.x)
        self.y = self._get_direction_as_value(directions["y"]) if self.y is None else self._get_direction_as_value(self.y)
        self.z = self._get_direction_as_value(directions["z"]) if self.z is None else self._get_direction_as_value(self.z)
