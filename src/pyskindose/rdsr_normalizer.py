...
def rdsr_normalizer(data_parsed: pd.DataFrame) -> pd.DataFrame:
    ...
    data_norm : pd.DataFrame
        DataFrame with the following columns
            ...
            - Ap3 (int)
                Rotation angle of the X-ray detector about the isocenter
                y-axis. Positive direction is determined by the right-hand rule
                for curve orientation about the positive isocenter y-axis.
            - DSL (float)
                Detector Side Length (DSL) in cm.
            - FS_lat (float)
                Side length of the X-ray field in the lateral direction at the
                image receptor plane.
            - FS_long (float)
                Side length of the X-ray field in the longitudinal direction at
                the image receptor plane.
            - kVp  (float)
                Tube voltage in kV
            - K_IRP  (float)
