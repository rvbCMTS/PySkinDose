...
def perform_calculations_for_new_geometries(normalized_data: pd.DataFrame, event: int, new_geometry: bool,
                                            patient: Phantom, table: Phantom, pad: Phantom,
                                            hits: List[bool], table_hits: List[bool], field_area: List[float],
                                            k_isq: np.array):
    if not new_geometry:
    ...
    patient.position(data_norm=normalized_data, i=event)
